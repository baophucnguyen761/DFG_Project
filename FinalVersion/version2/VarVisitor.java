...
//NameExpr example: int x=a+3
    ...
    public void writeToDot(int ID_code) {
        try {
            if (ID_code == 0) {
                ...
                //add the sinks and source nodes to this
    ...
    public void writeFormatDot(String item, String NodeTemp) {
        ...
        try (FileWriter writer = new FileWriter(pathDot.toFile(), true)) {
            if (!(NodeTemp.contains("VAnode"))){
                line2=parts[3];
                if (parts.length-1>3){
                    line3=parts[4];
                    writer.write(NodeTemp + " [ label=\""+"Line:"+ line + ","+line2+","+line3+ "\" ];\n");
                }else {
                    writer.write(NodeTemp + " [ label=\""+"Line:"+ line + ","+line2+ "\" ];\n");
                }
            }else {
                writer.write(NodeTemp + " [ label=\""+"Line:"+ line + ","+ "\" ];\n");
            }
        } catch (IOException e) {
    ...
    public boolean SScheck(String Stringsale) {
        for (int i = 0; i < source.length; i++) {
            if (Stringsale.contains(source[i])) {
                ...
                SS1int++;
                writeToDot(5);
                Stringsale=getNodeOp(Stringsale);
    ...
    public String getNodeOp(String item){
        String[] arithmetic={"+","-","*","/","%"};
        String[] relationalOperators = {"==", "!=", ">", "<", ">=", "<="};
        String[] logicalOperators = {"&&", "||", "!"};
        String[] assignmentOperators = {"=", "+=", "-=", "*=", "/=", "%="};
        String[] bitwiseOperators = {"&", "|", "^", "~", "<<", ">>"};
        String[] ops={"if","while","break","continue","for","do","try"};
        ...
        for (int i=0;i<assignmentOperators.length-1;i++){
            if(item.contains(assignmentOperators[i])){
                item=item +"|AO";
            }
        }
        for (int i=0;i<arithmetic.length-1;i++){
            if(line.contains(arithmetic[i])){
                item=item +"|A";
            }
        }
        for (int i=0;i<relationalOperators.length-1;i++){
            if(item.contains(relationalOperators[i])){
                item=item +"|R";
            }
        }
        for (int i=0;i<logicalOperators.length-1;i++){
            if(item.contains(logicalOperators[i])){
                item=item +"|L";
            }
        }
        for (int i=0;i<bitwiseOperators.length-1;i++){
            if(item.contains(bitwiseOperators[i])){
                item=item +"|B";
            }
        }
