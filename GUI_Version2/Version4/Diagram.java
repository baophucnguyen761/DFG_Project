import guru.nidi.graphviz.parse.Parser;  // Correct import for Graphviz Parser
import guru.nidi.graphviz.engine.Graphviz; // Graphviz rendering engine
import guru.nidi.graphviz.engine.Format;  // Graphviz file format (PNG, etc.)
import guru.nidi.graphviz.model.MutableGraph;  // Graph model from Graphviz
...
public class Diagram extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        grid.add(previousProjectField, 1, 4);
        grid.add(browsePreviousButton, 2, 4);
    ...
    private void proceed(Stage primaryStage) {
        ...
        // If Flow Chart is selected, show the flowchart
    ...
    private void displayDotFile(File file) {
        try {
            ...
            // Optionally, render the .dot file to a flowchart image
            renderDotToImage(file);
    ...
    // Optionally, render the .dot file content into an image
    private void renderDotToImage(File dotFile) {
        ...
        try {
            ...
            e.printStackTrace();
            fileService.showAlert("Error", "Failed to render .dot file to image.");
        }
    }
